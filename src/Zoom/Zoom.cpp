...
int main(int argc, char* argv[]) {
	...
	//若无参数
	if(argc == 1) {
		cout << "BMP Resizer, aka zomm, Written by Horizon Chaser\n" << endl;
		cout << "Usage:\n    zoom sourceBMP Scale(in percentage, like 404) destBMP\n" << endl;
		cout << "[ERROR] No Args Provided" << endl;
		return 0;
	}
	...
	inFile.open(argv[1]);
	if(!inFile) {
		cout << "[ERROR] Source File: " << argv[1] << " Not Found Or Can't Read" << endl;
		return -2;
	}
	inFile.close();
	...
	//计时用
	long timer = clock();
	bililinearBmpScaling(argv[1], argv[3], atoi(argv[2])/100.0);
	cout << "[INFO] Source BMP Successfully Resized to " << atoi(argv[2]) << "% in " << int(((double)(clock() - timer)) / CLOCKS_PER_SEC * 1000) << "ms" << endl;
	return 0;
}